-- [[ X ULTIMATE HACK V3.7.01 ]] --
-- UPDATE: Added Click TP [Ctrl+Click], ESP Chams (Highlight), and Spectate System.
-- Core: Team Check, Silent Aim (Ghost Head), Vehicle Smart Hitbox, NoFall.
-- UI: Cyber Tech Theme with Spectate Panel.
...
local Theme = {
    ...
    Stroke = Color3.fromRGB(0, 255, 255),    -- Enemy Color
    Team = Color3.fromRGB(0, 255, 100),      -- Team Color
...
-- 2. 核心状态
local States = {
    Aimbot = false, SilentAim = false, KillAll = false, Hitbox = false, TPToTarget = false,
    TriggerBot = false, TeamCheck = false,
    ESP = false, Chams = false, Tracers = false, XRay = false, Fullbright = false, Crosshair = false, -- [NEW: Chams]
    Fly = false, SpeedHack = false, InfJump = false, Noclip = false, ClickTP = false, -- [NEW: ClickTP]
    NoFall = false,
    AntiAim = false, Desync = false, ShowFOV = false
}

local Vals = {
    FOV = 200, WalkSpeed = 100, FlySpeed = 60, HitboxSize = 15, KillInterval = 0.1,
    SilentHeadSize = 25
}
        ...
        if p ~= LocalPlayer and p.Character then
            ...
            if States.Chams then
                ...
                if IsTeammate(p) then
                    highlight.FillColor = Theme.Team
                    highlight.OutlineColor = Theme.Team
                else
                    highlight.FillColor = Theme.Stroke
                    highlight.OutlineColor = Theme.Stroke
                end
            else
...
local function createESP(plr)
    ...
    local esp = {Box=Drawing.new("Square"),Name=Drawing.new("Text"),HealthBar=Drawing.new("Line"),HealthOutline=Drawing.new("Line")}
    ...
    esp.HealthOutline.Thickness=3; esp.HealthOutline.Color=Color3.new(0,0,0); esp.HealthOutline.Visible=false
...
-- [[ 4. UI SYSTEM (V3.7.01) ]] --
local guiName = "X_ULTIMATE_V3_7_01"
if targetGui:FindFirstChild(guiName) then targetGui[guiName]:Destroy() end
local ScreenGui = Instance.new("ScreenGui", targetGui)
ScreenGui.Name = guiName; ScreenGui.ResetOnSpawn = false

local Main = Instance.new("Frame", ScreenGui)
Main.Size = UDim2.new(0, 600, 0, 400); Main.Position = UDim2.new(0.5, -300, 0.5, -200); Main.BackgroundColor3 = Theme.Main; Main.Active = true; Main.Draggable = true
...
local Ver = Instance.new("TextLabel", SidePanel); Ver.Text = "V3.7.01"; Ver.Size = UDim2.new(1, 0, 0, 20); Ver.Position = UDim2.new(0, 0, 0, 35); Ver.BackgroundTransparency = 1; Ver.TextColor3 = Theme.TextDim; Ver.TextSize = 11; Ver.Font = Enum.Font.GothamBold
...
local function CreatePage(name)
    local Page = Instance.new("ScrollingFrame", PageHolder); Page.Size = UDim2.new(1, 0, 1, 0); Page.BackgroundTransparency = 1; Page.Visible = false; Page.ScrollBarThickness = 2; Page.ScrollBarImageColor3 = Theme.Stroke
    ...
    TabBtn.MouseButton1Click:Connect(function() for _,v in pairs(PageHolder:GetChildren()) do if v:IsA("ScrollingFrame") then v.Visible=false end end; for _,v in pairs(TabHolder:GetChildren()) do if v:IsA("TextButton") then TweenService:Create(v, TweenInfo.new(0.2), {BackgroundColor3=Color3.fromRGB(30,30,35), TextColor3=Theme.TextDim}):Play() end end; Page.Visible = true; TweenService:Create(TabBtn, TweenInfo.new(0.2), {BackgroundColor3=Theme.Stroke, TextColor3=Theme.Main}):Play() end)
    return Page, TabBtn
end
...
local function AddButton(page, text, cb)
    local Btn = Instance.new("TextButton", page); Btn.Size = UDim2.new(1, -5, 0, 40); Btn.BackgroundColor3 = Theme.Sec; Btn.Text = text; Btn.TextColor3 = Theme.Text; Btn.Font = Enum.Font.GothamBold; Btn.TextSize = 13; Btn.AutoButtonColor = false; Instance.new("UICorner", Btn).CornerRadius = UDim.new(0, 6)
    local Stroke = Instance.new("UIStroke", Btn); Stroke.Color = Theme.Stroke; Stroke.Transparency = 0.9
    Btn.MouseButton1Click:Connect(function() TweenService:Create(Btn, TweenInfo.new(0.1), {BackgroundColor3 = Theme.Stroke, TextColor3 = Theme.Main}):Play(); task.wait(0.1); TweenService:Create(Btn, TweenInfo.new(0.2), {BackgroundColor3 = Theme.Sec, TextColor3 = Theme.Text}):Play(); cb() end)
end
...
local function AddInputBox(page, text, placeholder, cb)
    local Frame = Instance.new("Frame", page); Frame.Size = UDim2.new(1, -5, 0, 50); Frame.BackgroundColor3 = Theme.Sec; Instance.new("UICorner", Frame).CornerRadius = UDim.new(0, 6)
    local Label = Instance.new("TextLabel", Frame); Label.Text = text; Label.Size = UDim2.new(1, -10, 0, 20); Label.Position = UDim2.new(0, 10, 0, 2); Label.BackgroundTransparency = 1; Label.TextColor3 = Theme.Text; Label.Font = Enum.Font.GothamBold; Label.TextSize = 12; Label.TextXAlignment = Enum.TextXAlignment.Left
    local Box = Instance.new("TextBox", Frame); Box.Size = UDim2.new(1, -20, 0, 22); Box.Position = UDim2.new(0, 10, 0, 24); Box.BackgroundColor3 = Color3.fromRGB(40,40,45); Box.Text = ""; Box.PlaceholderText = placeholder; Box.TextColor3 = Color3.new(1,1,1); Box.Font = Enum.Font.Gotham; Box.TextSize = 12; Instance.new("UICorner", Box).CornerRadius = UDim.new(0, 4)
...
-- Pages
local P1, T1 = CreatePage("COMBAT"); P1.Visible = true; T1.BackgroundColor3 = Theme.Stroke; T1.TextColor3 = Theme.Main
local P2, T2 = CreatePage("VISUAL")
local P3, T3 = CreatePage("MOVEMENT")
local P4, T4 = CreatePage("OTHER")

-- Items
AddToggle(P1, "Aimbot", "Aimbot")
AddToggle(P1, "Silent Aim (Assist)", "SilentAim")
AddToggle(P1, "Team Check (Safe)", "TeamCheck")
...
AddToggle(P2, "ESP Chams (Glow)", "Chams") -- [NEW]
...
AddToggle(P3, "Click TP [Ctrl+Click]", "ClickTP") -- [NEW]
...
-- [NEW] SPECTATE SECTION
local specName = ""
AddInputBox(P4, "Spectate Player", "Enter Name...", function(t) specName = t end)
AddDual(P4, "SPECTATE", function() SpectatePlayer(specName) end, "UNSPECTATE", function() StopSpectate() end)
AddToggle(P4, "Anti-Aim (Spin)", "AntiAim")
AddToggle(P4, "Desync (Smooth)", "Desync")
...
RunService.RenderStepped:Connect(function()
    ...
    if States.Chams then UpdateChams() end -- Update Highlights
    
    if States.ESP or States.Tracers then
        for plr, esp in pairs(ESPObjects) do
            local char = plr.Character; local root = char and char:FindFirstChild("HumanoidRootPart")
            if char and root and char:FindFirstChild("Head") and char:FindFirstChild("Humanoid").Health > 0 then
                local vector, onScreen = Camera:WorldToViewportPoint(root.Position)
                local drawColor = Theme.Stroke
                if States.TeamCheck and IsTeammate(plr) then drawColor = Theme.Team end
                ...
                if onScreen then
                    ...
                    if States.ESP then
                        esp.Box.Visible=true; esp.Box.Size=Vector2.new(width, height); esp.Box.Position=Vector2.new(vector.X - width/2, vector.Y - height/2); esp.Box.Color=drawColor
                        esp.Name.Visible=true; esp.Name.Text=plr.Name; esp.Name.Position=Vector2.new(vector.X, esp.Box.Position.Y - 18); esp.Name.Color=drawColor
                        esp.HealthBar.Visible=true; esp.HealthBar.From=Vector2.new(esp.Box.Position.X - 5, esp.Box.Position.Y + height); esp.HealthBar.To=Vector2.new(esp.Box.Position.X - 5, esp.Box.Position.Y + height - height*(char.Humanoid.Health/char.Humanoid.MaxHealth))
                    else esp.Box.Visible=false; esp.Name.Visible=false; esp.HealthBar.Visible=false end
                    if States.Tracers then
                        local l = tracerLines[plr] or Drawing.new("Line"); tracerLines[plr]=l; l.Visible=true; l.Thickness=1.5; l.Color=drawColor; l.From=Vector2.new(Camera.ViewportSize.X/2, Camera.ViewportSize.Y); l.To=Vector2.new(vector.X, vector.Y)
                    elseif tracerLines[plr] then tracerLines[plr].Visible=false end
                else esp.Box.Visible=false; esp.Name.Visible=false; esp.HealthBar.Visible=false; if tracerLines[plr] then tracerLines[plr].Visible=false end end
            else esp.Box.Visible=false; esp.Name.Visible=false; esp.HealthBar.Visible=false; if tracerLines[plr] then tracerLines[plr].Visible=false end end
        end
    end
    if States.Crosshair and Drawing then
        local c = Vector2.new(Camera.ViewportSize.X/2, Camera.ViewportSize.Y/2); crosshairLines.H.Visible=true; crosshairLines.H.From=Vector2.new(c.X-10, c.Y); crosshairLines.H.To=Vector2.new(c.X+10, c.Y); crosshairLines.V.Visible=true; crosshairLines.V.From=Vector2.new(c.X, c.Y-10); crosshairLines.V.To=Vector2.new(c.X, c.Y+10)
    elseif Drawing then crosshairLines.H.Visible=false; crosshairLines.V.Visible=false end
end)
...
RunService.Heartbeat:Connect(function()
    ...
    if States.SilentAim or States.Hitbox then
        for _,p in pairs(Players:GetPlayers()) do
            if p~=LocalPlayer and p.Character then
                if States.TeamCheck and IsTeammate(p) then continue end
                ...
                if eHum and eRoot and eHead then
                    if States.SilentAim then
                        eHead.Size=Vector3.new(Vals.SilentHeadSize, Vals.SilentHeadSize, Vals.SilentHeadSize); eHead.Transparency=1; eHead.CanCollide=false; eHead.Massless=true
                    else
                        if not eHum.Sit then eHead.Size=Vector3.new(1.2,1,1); eHead.Transparency=0; eHead.CanCollide=true; eHead.Massless=false end
                    end
                    if States.Hitbox then
                        if eHum.Sit then
                            if not States.SilentAim then eHead.Size=Vector3.new(Vals.HitboxSize,Vals.HitboxSize,Vals.HitboxSize); eHead.Transparency=0.7; eHead.CanCollide=false; eHead.Massless=true end
                            eRoot.Size=Vector3.new(2,2,1); eRoot.Transparency=1; eRoot.CanCollide=true; eRoot.Massless=false
                        else
                            eRoot.Size=Vector3.new(Vals.HitboxSize,Vals.HitboxSize,Vals.HitboxSize); eRoot.Transparency=0.7; eRoot.CanCollide=false; eRoot.Massless=true
                        end
                    else
                        eRoot.Size=Vector3.new(2,2,1); eRoot.Transparency=1; eRoot.CanCollide=true; eRoot.Massless=false
                    end
        ...
        for _,p in pairs(Players:GetPlayers()) do
            if p~=LocalPlayer and p.Character then
                local eRoot = p.Character:FindFirstChild("HumanoidRootPart"); local eHead = p.Character:FindFirstChild("Head")
                if eRoot then eRoot.Size=Vector3.new(2,2,1); eRoot.Transparency=1; eRoot.CanCollide=true; eRoot.Massless=false end
                if eHead then eHead.Size=Vector3.new(1.2,1,1); eHead.Transparency=0; eHead.CanCollide=true; eHead.Massless=false end
            end
        end
    end
    
    if States.Fly then
        hum.PlatformStand = true; local dir = Vector3.zero; local cf = Camera.CFrame
        ...
        if UIS:IsKeyDown(Enum.KeyCode.E) then dir = dir + Vector3.new(0,1,0) end; if UIS:IsKeyDown(Enum.KeyCode.Q) then dir = dir - Vector3.new(0,1,0) end
        hrp.AssemblyLinearVelocity = dir * Vals.FlySpeed
        if States.Desync then hrp.AssemblyLinearVelocity = hrp.AssemblyLinearVelocity + Vector3.new(math.random(-50,50),0,math.random(-50,50)) end
    else
        if hum.PlatformStand then hum.PlatformStand = false end
        if States.Desync then hrp.AssemblyLinearVelocity = hrp.AssemblyLinearVelocity + Vector3.new(math.random(-40,40),0,math.random(-40,40)) end
    end
    
    if States.AntiAim then hrp.CFrame = hrp.CFrame * CFrame.Angles(0, math.rad(20), 0) end
    if States.InfJump and UIS:IsKeyDown(Enum.KeyCode.Space) then hum:ChangeState(Enum.HumanoidStateType.Jumping) end
    if States.SpeedHack then hum.WalkSpeed = Vals.WalkSpeed end
    if States.TPToTarget then local t = getClosestToCenter(); if t then hrp.CFrame = t.Parent.HumanoidRootPart.CFrame * CFrame.new(0,0,3); hrp.CFrame = CFrame.lookAt(hrp.Position, t.Position) end; States.TPToTarget = false end
end)

-- [[ CLICK TP HANDLER (NEW) ]] --
...
print("X ULTIMATE V3.7.01 UPGRADED LOADED")
